...
def create_application(
    ...
    added = add_application(current_user.id, job_id, session)
    if not added:
        raise HTTPException(status_code=status.HTTP_400_BAD_REQUEST, detail="Failed to link application to user")
    return application

@router.get("/{application_id}", response_model=ApplicationResponse)
def get_application(application_id: UUID, current_user: User = Depends(get_current_user), session: Session = Depends(db_session_manager.get_session)):
...
def get_applications_by_job(job_id: UUID, current_user: User = Depends(get_current_user), session: Session = Depends(db_session_manager.get_session)):
    job = get_job_by_id(job_id, session)
    if not job:
        raise HTTPException(status_code=status.HTTP_404_NOT_FOUND, detail="Job not found")
...
def get_applications_by_user(user_id: UUID, current_user: User = Depends(get_current_user), session: Session = Depends(db_session_manager.get_session)):
...
def update_application_status(application_id: UUID, status: ApplicationStatus, current_user: User = Depends(get_current_user), session: Session = Depends(db_session_manager.get_session)):
    if current_user.role != UserRole.RECRUITER:
    ...
    if not job:
        raise HTTPException(status_code=status.HTTP_404_NOT_FOUND, detail="Job not found")
    updated_application = update_application(application_id, ApplicationUpdate(status=status), session)
...
def delete_application_api(application_id: UUID, current_user: User = Depends(get_current_user), session: Session = Depends(db_session_manager.get_session)):
    ...
    removed= remove_application(application_id, application.job_id, session)
    if not removed:

...
def create_job(job: JobCreate, current_user: User = Depends(get_current_user), session: Session = Depends(db_session_manager.get_session)):
...
def get_job(job_id: UUID, session: Session = Depends(db_session_manager.get_session)):
...
def update_job(job_id: UUID, job: JobUpdate, current_user: User = Depends(get_current_user), session: Session = Depends(db_session_manager.get_session)):
    if current_user.role != UserRole.RECRUITER:
...
def delete_job(job_id: UUID, current_user: User = Depends(get_current_user), session: Session = Depends(db_session_manager.get_session)):
    if current_user.role != UserRole.RECRUITER:

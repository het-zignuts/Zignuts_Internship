...
def add_application(application_id: UUID, job_id: UUID, session: Session) -> bool:
    application=session.exec(select(Application).where(Application.id==application_id)).first()
    job=session.exec(select(Job).where(Job.id==job_id)).first()
    if not application or not job:
        return False
    if application_id not in job.applications:
        job.applications.append(application_id)
    session.add(job)
    session.commit()
    return True

def remove_application(application_id: UUID, job_id: UUID, session: Session) -> bool:
    application=session.exec(select(Application).where(Application.id==application_id)).first()
    job=session.exec(select(Job).where(Job.id==job_id)).first()
    if not application or not job:
        return False
    if application_id in job.applications:
        job.applications.remove(application_id)
    session.add(job)
    session.commit()
    return True